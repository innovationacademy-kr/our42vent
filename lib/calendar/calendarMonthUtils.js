...
export async function getHolidayArray(year, month) {
  const url = 'http://apis.data.go.kr/B090041/openapi/service/SpcdeInfoService/getRestDeInfo'; // URL
  let queryParams = `?${encodeURIComponent('serviceKey')}=${process.env.OPEN_API_SERVICE_KEY}`; // Service Key
  queryParams += `&${encodeURIComponent('solYear')}=${encodeURIComponent(year)}`;
  queryParams += `&${encodeURIComponent('solMonth')}=${0 + encodeURIComponent(month)}`;

  try {
    const res = await axios.get(url + queryParams, { headers: { Accept: 'application/xml' } });
    consoleLogger.info(
      `Status: ${res.status} nBody: ${res.data.match(/<locdate>(.*?)<\/locdate>/g)}`
    );
  } catch (err) {
