...
export default async function triggerPush() {
  try {
    const now = new Date();
    const sendAt = new Date(now.setMinutes(now.getMinutes() + 1, 0, 0));
    const pushInfoArray = await selectNextNotifications(sendAt);

    pushInfoArray.forEach(info => {
      const { eventId, title, sub, notification } = info;
